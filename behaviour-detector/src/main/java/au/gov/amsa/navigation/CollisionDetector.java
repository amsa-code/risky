...
public class CollisionDetector {
	...
	private static Func2<State, VesselPosition, State> nextState() {
		return new Func2<State, VesselPosition, State>() {
			@Override
			public State call(State state, VesselPosition p) {
				return state.nextState(MAX_TIME_INTERVAL_MS, p);
			}
		};
	}

	private static Func1<State, Observable<CollisionCandidate>> toCollisionCandidatesForPosition() {
		return new Func1<State, Observable<CollisionCandidate>>() {
			@Override
			public Observable<CollisionCandidate> call(State state) {
	...
	private static Func1<GroupedObservable<IdentifierPair, CollisionCandidate>, Observable<? extends CollisionCandidate>> onlyRepeating() {
		return new Func1<GroupedObservable<IdentifierPair, CollisionCandidate>, Observable<? extends CollisionCandidate>>() {
	...
	private static Func1<GroupedObservable<Identifier, VesselPosition>, Observable<TreeSet<VesselPosition>>> toSortedSet() {
		return new Func1<GroupedObservable<Identifier, VesselPosition>, Observable<TreeSet<VesselPosition>>>() {
	...
	private static Func1<VesselPosition, Identifier> byId() {
		return new Func1<VesselPosition, Identifier>() {

			@Override
			public Identifier call(VesselPosition position) {
				return position.id();
			}
		};
	}
	...
	private static Func1<VesselPosition, Boolean> aroundInTime(
		...
		return new Func1<VesselPosition, Boolean>() {
			...
			public Boolean call(VesselPosition p) {
				return Math.abs(p.time() - position.time()) <= maxTimeIntervalMs;
			}
		};
	}
