...
public class AisVesselPositions {
	...
	public static Transformer<String, VesselPosition> positions() {
		return new Transformer<String, VesselPosition>() {
			@Override
			public Observable<VesselPosition> call(Observable<String> nmea) {
				return positions(nmea);
			}
		};
	}
	...
	public static Observable<TimestampedAndLine<AisMessage>> sortByTime(
		...
		Comparator<TimestampedAndLine<AisMessage>> comparator = new Comparator<TimestampedAndLine<AisMessage>>() {
			@Override
			public int compare(TimestampedAndLine<AisMessage> t1, TimestampedAndLine<AisMessage> t2) {
				return ((Long) t1.getMessage().get().time())
				        .compareTo(t2.getMessage().get().time());
			}
		};
		return source
		// sort by time
	...
	private static final Func1<AisMessageAndVesselData, Observable<VesselPosition>> toVesselPosition = new Func1<AisMessageAndVesselData, Observable<VesselPosition>>() {
		@Override
		public Observable<VesselPosition> call(AisMessageAndVesselData messageAndData) {
			...
			if (messageAndData.message().isPresent()
				...
				if (p.getCourseOverGround() == null || p.getTrueHeading() == null
					...
					Optional<Integer> widthMetres = vessel.isPresent() ? vessel.get()
					        .getWidthMetres() : Optional.<Integer> absent();
