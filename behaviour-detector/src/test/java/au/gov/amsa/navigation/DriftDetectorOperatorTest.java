...
public class DriftDetectorOperatorTest {
	...
	public void testDrifted() {
		...
		Mockito.when(fix.courseOverGroundDegrees()).thenReturn(
				Optional.of(10.0f));
		Mockito.when(fix.headingDegrees()).thenReturn(Optional.of(110.0f));
		Mockito.when(fix.speedOverGroundKnots()).thenReturn(
				Optional.of(DRIFT_SPEED_KNOTS));
		Mockito.when(fix.navigationalStatus()).thenReturn(
				Optional.<NavigationalStatus> absent());
		assertTrue(DriftDetectorOperator.isCandidate(Options.instance()).call(
				fix));
	}
	...
	public void testDriftedFalseIfNoCog() {
		...
		Mockito.when(fix.navigationalStatus()).thenReturn(
				Optional.<NavigationalStatus> absent());
		assertFalse(DriftDetectorOperator.isCandidate(Options.instance()).call(
				fix));
	}
	...
	public void testDriftedFalseIfNoHeading() {
		...
		Mockito.when(fix.courseOverGroundDegrees()).thenReturn(
				Optional.of(10.0f));
		Mockito.when(fix.headingDegrees())
				.thenReturn(Optional.<Float> absent());
		Mockito.when(fix.speedOverGroundKnots()).thenReturn(
				Optional.of(DRIFT_SPEED_KNOTS));
		Mockito.when(fix.navigationalStatus()).thenReturn(
				Optional.<NavigationalStatus> absent());
	...
	public void testRule2TwoDriftersBigTimeGap() {
		...
		Fix f1 = createFix(90, DRIFT_SPEED_KNOTS, t);
		// drifter
		Fix f2 = createFix(91, DRIFT_SPEED_KNOTS,
	...
	public void testRule3FourDriftersBigTimeGapBetweenTwoAndThree() {
		...
		Fix f2 = createFix(91, DRIFT_SPEED_KNOTS, t += 1);
		// drifter
		Fix f3 = createFix(92, DRIFT_SPEED_KNOTS,
				t += testOptions.expiryAgeMs() + 1);
		// drifter
	...
	public void testRule6DrifterNonDrifterDrifterOverNonDriftingThreshold() {
		...
		Fix f2 = createFix(1, DRIFT_SPEED_KNOTS, t += 1);
		// drifter
		Fix f3 = createFix(91, DRIFT_SPEED_KNOTS,
				t += testOptions.nonDriftingThresholdMs() + 1);
		List<DriftCandidate> list = getCandidates(Observable.just(f1, f2, f3));
		assertTrue(list.isEmpty());
	}
	...
	public void testBackpressure() {
		TestSubscriber<DriftCandidate> ts = new TestSubscriber<DriftCandidate>(
				1) {
			@Override
		...
		Observable.just(f1, f2, f3, f4, f5)
				.compose(DriftDetector.detectDrift(testOptions)).subscribe(ts);
	...
	private List<DriftCandidate> getCandidates(Observable<Fix> source) {
		return source.compose(DriftDetector.detectDrift(testOptions)).toList()
				.toBlocking().single();
	}

	private static Fix createFix(long mmsi, float courseHeadingDiff,
			float speedKnots, long time) {
		Fix f = Mockito.mock(Fix.class);
		Mockito.when(f.courseOverGroundDegrees())
				.thenReturn(Optional.of(10.0f));
		Mockito.when(f.headingDegrees()).thenReturn(
				Optional.of(10.0f + courseHeadingDiff));
		Mockito.when(f.speedOverGroundKnots()).thenReturn(
				Optional.of(speedKnots));
		Mockito.when(f.navigationalStatus()).thenReturn(
				Optional.<NavigationalStatus> absent());
	...
	private static Fix createFix(float courseHeadingDiff, float speedKnots,
			long time) {
