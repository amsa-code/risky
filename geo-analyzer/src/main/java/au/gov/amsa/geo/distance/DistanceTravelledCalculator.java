...
public class DistanceTravelledCalculator {
	...
	private static Logger log = Logger
			.getLogger(DistanceTravelledCalculator.class);

	private final Options options;
	private final DistanceCalculationMetrics metrics;

	public DistanceTravelledCalculator(Options options,
	...
	private static boolean timeDifferenceOk(Fix a, Fix b, SegmentOptions o) {
		...
		return o.maxTimePerSegmentMs() == null
				|| timeDiffMs <= o.maxTimePerSegmentMs();
	}
	...
	private final Func1<? super List<Fix>, ? extends Observable<? extends CellAndDistance>> toCellAndDistance = new Func1<List<Fix>, Observable<CellAndDistance>>() {
	...
	@VisibleForTesting
	static final Observable<CellAndDistance> getCellDistances(HasPosition a,
			HasPosition b, Options options) {
		return getCellDistances(Util.toPos(a), Util.toPos(b), options);
	}

	@VisibleForTesting
	static final Observable<CellAndDistance> getCellDistances(final Position a,
			final Position b, final Options options) {
			...
			public void call(Subscriber<? super CellAndDistance> subscriber) {
				try {
					...
					while (keepGoing) {
						...
						Optional<Cell> cell = Cell.cellAt(p1.getLat(),
								p1.getLon(), options);
						if (cell.isPresent())
							subscriber.onNext(new CellAndDistance(cell.get(),
									distanceNm));
	...
	private static void checkCount(Position p1, Position destination,
			int count, Options options) {
		if (count > 100000)
			throw new RuntimeException("unexpectedly stuck in loop p1=" + p1
	...
	public static class CalculationResult {
		...
		public CalculationResult(Observable<CellValue> cells,
				DistanceCalculationMetrics metrics) {
			this.cells = cells;
			this.metrics = metrics;
		}
	...
	public static Observable<CellValue> calculateDensityByCellFromFiles(
			Options options, Observable<File> files, int horizontal,
			int vertical, DistanceCalculationMetrics metrics) {
	...
	private static Func1<Options, Observable<CellValue>> calculateDistanceTravelled(
			final Observable<File> files,
			final DistanceCalculationMetrics metrics) {
		return new Func1<Options, Observable<CellValue>>() {
			...
			public Observable<CellValue> call(Options options) {
				...
				DistanceTravelledCalculator c = new DistanceTravelledCalculator(
						options, metrics);
				...
				return Observable.from(c
						.calculateDistanceByCellFromFiles(files)
	...
	public static CalculationResult calculateTrafficDensity(Options options,
		...
		int maxNumCells = (int) Math.round(options.getBounds()
				.getWidthDegrees()
		...
		final Observable<CellValue> cells = DistanceTravelledCalculator
				.calculateDensityByCellFromFiles(options, files, horizontal,
						vertical, metrics);
		return new CalculationResult(cells, metrics);
	}

	private static Func1<CellAndDistance, CellValue> toCellDensityValue(
			final Options options) {
		return new Func1<CellAndDistance, CellValue>() {

			@Override
			public CellValue call(CellAndDistance cd) {
				return new CellValue(cd.getCell().getCentreLat(options), cd
						.getCell().getCentreLon(options),
						cd.getTrafficDensity(options));
			}
		};
	}
	...
	public static void saveCalculationResultAsText(Options options,
		...
		try {
			...
			out.format("%s\t%s\t%s\t%s\t%s\t%s\t%s\n", options.getOriginLat(),
					options.getOriginLon(), options.getCellSizeDegrees(),
					b.getTopLeftLat(), b.getTopLeftLon(),
					b.getBottomRightLat(), b.getBottomRightLon());
			...
			calculationResult.getCells().subscribe(new Observer<CellValue>() {
				...
				public void onNext(CellValue cell) {
					out.format("%s\t%s\t%s\n", cell.getCentreLat(),
							cell.getCentreLon(), cell.getValue());
				}
			});
	 ...
	 * @return
	 */
	public static Observable<Options> partition(final Options options,
				...
				for (int i = 0; i < horizontal; i++) {
					for (int j = 0; j < vertical; j++) {
						...
						if (subscriber.isUnsubscribed())
							return;
						log.info("emitting options " + count + " of "
								+ horizontal * vertical);
	...
	private OperatorMapEntries<Cell, AtomicDouble, CellAndDistance> emitMapEntries() {
		return new OperatorMapEntries<Cell, AtomicDouble, CellAndDistance>(
				TO_CELL_DISTANCE);
	}
