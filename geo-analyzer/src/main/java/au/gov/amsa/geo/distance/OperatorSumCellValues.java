...
public class OperatorSumCellValues implements Operator<CellValue, CellValue> {
	...
	public OperatorSumCellValues(boolean useDisk) {
		if (useDisk)
			map = MapDb.INSTANCE.getDb().getHashMap(
	...
	public Subscriber<? super CellValue> call(
			final Subscriber<? super CellValue> child) {
				...
				.from(new Observer<CellValue>() {
					...
					public void onCompleted() {
						...
						try {
							...
							synchronized (map) {
								for (Entry<Position, AtomicDouble> entry : map
										.entrySet()) {
									CellValue cv = new CellValue(entry.getKey()
											.getLat(), entry.getKey().getLon(),
					...
					public void onNext(CellValue cv) {
						...
						Position position = new Position(cv.getCentreLat(), cv
								.getCentreLon());
