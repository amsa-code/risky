...
public class Bounds {
	...
	private final double topLeftLat, topLeftLon, bottomRightLat,
			bottomRightLon;

	public Bounds(double topLeftLat, double topLeftLon, double bottomRightLat,
	...
	public boolean contains(HasPosition p) {
		return contains(p.getPosition().getLat(), p.getPosition().getLon());
	}

	public boolean contains(double lat, double lon) {
		return topLeftLat >= lat
				&& bottomRightLat <= lat
				&& betweenLongitudes(topLeftLon, bottomRightLon,
						Position.to180(lon));
	}
	 ...
	 * @return
	 */
	private static boolean betweenLongitudes(double topLeftLon,
			double bottomRightLon, double lon) {
	...
	public Bounds expand(double latExpansionDegrees, double lonExpansionDegrees) {
		return new Bounds(Math.min(90, topLeftLat + latExpansionDegrees),
				topLeftLon - lonExpansionDegrees, Math.max(-90, bottomRightLat
						- latExpansionDegrees), bottomRightLon
						+ lonExpansionDegrees);
	}
