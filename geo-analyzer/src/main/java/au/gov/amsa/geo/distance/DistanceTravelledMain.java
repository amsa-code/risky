...
public class DistanceTravelledMain {
	...
	private static void run(String directory, Options options, boolean gui) {
		...
		CalculationResult resultFromFile = new CalculationResult(
				BinaryCellValuesObservable.readValues(new File(filename))
						.skip(1).cast(CellValue.class), result.getMetrics());
		// 8:5 is ok ratio
		saveAsPng(Renderer.createImage(options, 2, 1280, resultFromFile),
				new File("target/map.png"));

		// saveCalculationResultAsText(options, result, "target/densities.txt");
		// saveCalculationResultAsBinary(options, result,
