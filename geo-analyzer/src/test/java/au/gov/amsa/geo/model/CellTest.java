...
public class CellTest {
	...
	private static final Options options = Options.builder().originLat(0)
			.originLon(0).cellSizeDegrees(1.0)
	...
	public void testLeftEdgeLongitude() {
		...
		assertEquals(f1.getPosition().getLon(),
				cell.leftEdgeLongitude(options), PRECISION);
	}

	@Test
	public void testTopEdgeLatitude() {
		Cell cell = Cell.cellAt(f1, options).get();
		assertEquals(f1.getPosition().getLat(), cell.topEdgeLatitude(options),
				PRECISION);
	...
	public void testPositionAtLeftEdgeShouldNotResolveToTheCellToItsLeft() {
		...
		Cell cell2 = Cell.cellAt(0, cell.leftEdgeLongitude(options), options)
				.get();
	...
	public void testAreaInNauticalMilesOfOneDegreeCellNearEquator() {
		Options options = Options.builder().originLat(0).originLon(0)
				.cellSizeDegrees(1.0).bounds(new Bounds(0, 100, -60, 175))
				.build();
		Cell cell = Cell.cellAt(0, 142, options).get();
		assertEquals(3604.6847966219016, cell.areaNauticalMiles(options),
				PRECISION);
	}
	...
	public void testAreaInNauticalMilesOfOneDegreeCellAt45South() {
		...
		assertEquals(2526.6531760439616, cell.areaNauticalMiles(options),
				PRECISION);
	}
}
