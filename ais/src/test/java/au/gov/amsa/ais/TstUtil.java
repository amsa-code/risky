...
public class TstUtil {
	...
	static void handleAisStream(InputStream is,
			PrintStream messagesWithTimestamp, PrintStream out,
		...
		while ((line = br.readLine()) != null) {
			out.println(line);
			messagesWithTimestamp.println(line + ","
			...
			try {
				...
				if (message.message().getMessageId() <= 3 && message.message().getMessageId() > 0) {
					Communications comms = ((HasCommunications) message)
							.getCommunications();
					if (comms.getMinutesUtc() != null) {
						...
						String output = "comms,arrival=" + commsTime + ","
								+ arrivalTime;
	...
	static NmeaStreamProcessorListener createLoggingListener(
		...
		return new NmeaStreamProcessorListener() {
			...
			public void timestampNotFound(String line, Long arrivalTime) {
				...
				out.println(count++ + " " + line + " -> "
						+ new Date(arrivalTime) + " NOT FOUND " + className
						+ " " + commsDate);
			}

			@Override
			public void invalidNmea(String line, long arrivalTime,
					String message) {
	...
	static void process(NmeaReader con, NmeaStreamProcessorListener listener,
			final PrintStream out) {
		...
		for (String line : con.read()) {
			...
			long time = Long.parseLong(nmea.getItems().get(
					nmea.getItems().size() - 1));
