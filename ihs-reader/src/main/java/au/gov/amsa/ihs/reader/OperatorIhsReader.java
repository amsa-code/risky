...
public class OperatorIhsReader implements Operator<Ship, InputStream> {
	...
	public Subscriber<? super InputStream> call(
			final Subscriber<? super Ship> child) {
	...
	private static DefaultHandler createHandler(
			final Subscriber<? super Ship> subscriber) {
			...
			public void endElement(String uri, String localName, String qName)
				...
				if (ELEMENT_SHIP_DATA.equals(qName)) {
					try {
						...
						throw new RuntimeException("error building Ship from "
								+ values, e);
					}
				}
			}
			...
			private Ship buildShip() {
				return Ship
						...
						.classificationSocietyCode(
								value("ClassificationSocietyCode"))
						...
						.groupBeneficialOwnerCompanyCode(
								value("GroupBeneficialOwnerCompanyCode"))
						...
						.type2(value("ShiptypeLevel2"))
						.type3(value("ShiptypeLevel3"))
						.type4(value("ShiptypeLevel4"))
						.type5(value("ShiptypeLevel5"))
						...
						.lastUpdateTime(toDateTime(value("LastUpdateDate")))
						// build it
						.build();
			}
