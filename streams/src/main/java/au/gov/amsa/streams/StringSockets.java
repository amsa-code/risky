...
public final class StringSockets {
    ...
    private static Func1<Integer, Observable<String>> from(final String host, final int port,
            final Charset charset, final long quietTimeoutMs) {
    ...
    @VisibleForTesting
    static Func0<Socket> socketCreator(final String host, final int port, final long quietTimeoutMs) {
    ...
    static Func1<Socket, Observable<String>> socketObservableFactory(final Charset charset) {
        return new Func1<Socket, Observable<String>>() {
            ...
            public Observable<String> call(Socket socket) {
                try {
                    return StringObservable.from(new InputStreamReader(socket.getInputStream(),
                            charset));
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
